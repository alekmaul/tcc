                       ...
                       const char *name)
{
    ...
    do_def:
        sym_index
            = put_elf_sym(s, value, size, ELF32_ST_INFO(sym_bind, sym_type), other, sh_num, name);
    }
    return sym_index;
}
...
static void tcc_output_binary(TCCState *s1, FILE *f, const int *section_order)
{
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        if (s == text_section) {
            ...
            for (j = 0, esym = (Elf32_Sym *) symtab_section->data;
                 j < symtab_section->sh_size / sizeof(Elf32_Sym);
                 esym++, j++) {
          ...
          for(;ps;ps=ps->prev) {
            ...
            for (k = startk; k < endk; k++) {
                ...
                for (j = 0; j < size; j++) {
                    ...
                    for (ps = 0, esym = (Elf32_Sym *) symtab_section->data;
                         ps < symtab_section->sh_size / sizeof(Elf32_Sym);
                         esym++, ps++) {
                ...
                for (sym = (Elf32_Sym *) symtab_section->data + 1; sym < sym_end; sym++) {
                    if (sym->st_shndx == SHN_UNDEF) {
                        ...
                        if (sym_index) {
                            ...
                            /* XXX: _fp_hw seems to be part of the ABI, so we ignore
                               it */
                            if (ELF32_ST_BIND(sym->st_info) == STB_WEAK || !strcmp(name, "_fp_hw")) {
                            } else {
                                error_noabort("undefined symbol '%s'", name);
                            }
                        }
