...
#endif
#elif defined(TCC_UCLIBC) || defined(__FreeBSD__) || defined(__DragonFly__) || defined(__OpenBSD__)
/* currently incorrect */
    ...
    for (;;) {
        ...
        for (sym = global_stack; sym != NULL; sym = sym->prev) {
            ...
            if (((type->t & VT_BTYPE) == VT_FUNC)
                && (type->t & (VT_STATIC | VT_INLINE)) == (VT_STATIC | VT_INLINE) && sym->c != 0) {
...
{
    add_elf_sym(symtab_section, val, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, SHN_ABS, name);
    return 0;
}
