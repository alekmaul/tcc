...
char* get_sym_str(const Sym* sym) {
    ...
    if (sym->type.t & VT_STATIC) {
        if ((sym->type.t & VT_IMPORT) && current_fn[0] != 0 && !((sym->type.t & VT_BTYPE) == VT_FUNC)) {
            offset += snprintf(name + offset, sizeof(name) - offset, "%s_FUNC_%s_", static_prefix, current_fn);
        }
        else {
        ...
        else if (v == VT_CONST)
        {
            ...
            { // deref symbol + displacement
                ...
                if (is_float(ft))
                {
                    ...
                    {
                    case 4:
                        pr("lda.l %s + %d\nsta.b tcc__f%d\nlda.l %s + %d + 2\nsta.b tcc__f%dh\n", sy, fc, r - TREG_F0, sy, fc, r - TREG_F0);
                        break;
                    default:
                        error("ICE 1");
                    }
            ...
            else
            {
                ...
                { // value of local at fc
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        pr("lda %d + __%s_locals + 1,s\nsta.b tcc__r%d\n", fc + args_size, current_fn, r);
                        break;
                    case 4:
                        pr("lda %d + __%s_locals + 1,s\nsta.b tcc__r%d\nlda %d + __%s_locals + 1,s\nsta.b tcc__r%dh\n", fc + args_size, current_fn, r, fc + args_size + 2, current_fn, r);
                        break;
                    default:
                        error("ICE 2");
                        break;
        ...
        if (v == VT_LOCAL)
        {
            ...
            { // is_float(ft)) {
                if (base < 0)
                {
                    ...
                    {
                    case 4:
                        pr("lda.b tcc__f%d\nsta %d + __%s_locals + 1,s\nlda.b tcc__f%dh\nsta %d + __%s_locals + 1,s\n", r - TREG_F0, fc + args_size, current_fn, r - TREG_F0, fc + args_size + 2, current_fn);
                        break;
                    default:
                        error("ICE 6f");
                        break;
            ...
            {
                if (base < 0)
                { // write to local at fc
                    pr("; st%d tcc__r%d, [sp,%d]\n", length, r, fc);
                    fc = adjust_stack(fc, args_size + 2);
                    switch (length)
                    {
                    case 1:
                        pr("sep #$20\nlda.b tcc__r%d\nsta %d + __%s_locals + 1,s\nrep #$20\n", r, fc + args_size, current_fn);
                        break;
                    case 2:
                        pr("lda.b tcc__r%d\nsta %d + __%s_locals + 1,s\n", r, fc + args_size, current_fn);
                        break;
                    case 4:
                        pr("lda.b tcc__r%d\nsta %d + __%s_locals + 1,s\nlda.b tcc__r%dh\nsta %d + __%s_locals + 1,s\n", r, fc + args_size, current_fn, r, fc + args_size + 2, current_fn);
                        break;
                    default:
                        error("ICE 6");
                        break;
...
void gfunc_call(int nb_args)
{
    ...
    if (vtop->r & VT_LVAL)
    {
        ...
        { // call a symbolic function pointer
...
int gjmp(int t)
{
    ...
    {
        if (jump[i][0] == t)
        { // the jump target is a jump itself; make it go to same place as this one
            jump[i][0] = r;
        }
    }
    jumps++;
...
void gen_opi(int op)
{
    ...
    case '*':
        ...
        if (!skipcall)
        { // when no optim done, do the call
            pr("lda.b tcc__r%d\nsta.b tcc__r10\n", r);
            pr("jsr.l tcc__mul\n");
        }
        pr("sta.b tcc__r%d\n", r);
