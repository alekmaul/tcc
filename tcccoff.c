...
int tcc_output_coff(TCCState *s1, FILE *f)
{
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        if (do_debug && tcc_sect == stext) {
            ...
            char func_name[MAX_FUNC_NAME_LENGTH], last_func_name[MAX_FUNC_NAME_LENGTH];
            ...
            while (sym < sym_end) {
                ...
                case N_FUN:
                    if (sym->n_strx == 0) {
                        ...
                        FuncEntries[nFuncs] = (file_pointer - LineNoFilePtr[nFuncs]) / LINESZ - 1;
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        if (OutputTheSection(tcc_sect)) {
            // put relocations data
            if (coff_sec->s_nreloc > 0) {
                fwrite(tcc_sect->reloc, coff_sec->s_nreloc * sizeof(struct reloc), 1, f);
            }
        }
    }
            ...
            while (sym < sym_end) {
                ...
                case N_FUN:
                    if (sym->n_strx == 0) {
                        ...
                        CoffLineNo.l_addr.l_symndx = FindCoffSymbolIndex(func_name);
                        CoffLineNo.l_lnno = 0;
            ...
            for (j = 0; j < nb_syms; j++) {
                if (p2->st_info == 0x12) {
                    ...
                    if (k >= nFuncs) {
                        ...
                        sprintf(s, "debug (sort) info can't find function: %s", name2);
