...
#define MKTYPE(basic, d1, d2, d3, d4, d5, d6) \
    ((basic) | ((d1) << 4) | ((d2) << 6) | ((d3) << 8) | ((d4) << 10) | ((d5) << 12) | ((d6) << 14))
...
#define ISINT(x) \
    (((x) >= T_CHAR && (x) <= T_LONG) || ((x) >= T_UCHAR && (x) <= T_ULONG) || (x) == T_ENUM)
...
#define INCREF_COFF(x) \
    ((((x) & ~N_BTMASK_COFF) << N_TSHIFT_COFF) | (DT_PTR << N_BTSHFT_COFF) | (x & N_BTMASK_COFF))
