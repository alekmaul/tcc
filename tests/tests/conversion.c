// some parts of this test have been manipulated to forgive some ineptitude
// on the part of TCC in handling FP conversions
...
test_float_to_longlong_integer()
{
    ...
    // TCC has problems with complicated constant expressions, at least if they are long longs
    if (f2ull((float) 0x7fffffffULL) != (~0ULL) >> 1 && /* 0x7fffffff */
        f2ull((float) 0x7fffffffULL) != ((~0ULL) >> 1) + 1)
        abort();
    if (f2ull((float) (0x80000000ULL)) != ~((~0ULL) >> 1)) /* 0x80000000 */
        abort();
#if 0
  ...
  if (f2ull((float) ~((~0ULL) >> 1)) != ~((~0ULL) >> 1)) /* 0x80000000 */
    ...
    if (d2ull(1.99) != 1LL)
        abort();
    if (d2ull((double) 0x7fffffffULL) != (~0ULL) >> 1 && /* 0x7fffffff */
        d2ull((double) 0x7fffffffULL) != ((~0ULL) >> 1) + 1)
        abort();
    if (d2ull((double) 0x80000000ULL) != ~((~0ULL) >> 1)) /* 0x80000000 */
        abort();
#if 0
