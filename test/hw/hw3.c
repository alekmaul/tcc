...
const int __attribute__((section(".rodata"))) e[10]
    = {99, 98, 97, 96}; // ROM (together with string constants)
#endif
...
#define gen_testop(type)                                 \
    void testop_##type(type a, char *op, type b, type c) \
    {                                                    \
        write(#type " ");                                \
        writeequation(a, op, b);                         \
        hexus(c);                                        \
        OUTC('\n');                                      \
    }

gen_testop(int) gen_testop(__uint) gen_testop(char) gen_testop(__uchar) gen_testop(
    __longlong) gen_testop(__ulonglong) gen_testop(float)

#define gen_testunaryop(type)                         \
    void testunaryop_##type(type a, char *op, type b) \
    {                                                 \
        write(#type " ");                             \
        write(op);                                    \
        hex(a);                                       \
    ...
    gen_testunaryop(int) gen_testunaryop(__uint) gen_testunaryop(char) gen_testunaryop(
        __uchar) gen_testunaryop(__longlong) gen_testunaryop(__ulonglong) gen_testunaryop(float)

#define gen_test(op, sop, type)             \
    void test##op##type(type a, type b)     \
    {                                       \
        testop_##type(a, #sop, b, a sop b); \
    }

#define gen_testnotzero(op, sop, type)          \
    void test##op##type(type a, type b)         \
    {                                           \
        if (b)                                  \
            testop_##type(a, #sop, b, a sop b); \
...
#define gen_testunary(op, sop, type)        \
    void test##op##type(type a)             \
    {                                       \
        testunaryop_##type(a, #sop, sop a); \
    }

#define gen_testconst(op, sop, cst, type)           \
    void testconst##op##type(type a)                \
    {                                               \
...
#define gen_testall(op, sop)                                                 \
    gen_test(op, sop, int) gen_test(op, sop, __uint) gen_test(op, sop, char) \
        gen_test(op, sop, __uchar) gen_test(op, sop, __longlong) gen_test(op, sop, __ulonglong)

#define gen_testallnotzero(op, sop)                                                               \
    gen_testnotzero(op, sop, int) gen_testnotzero(op, sop, __uint) gen_testnotzero(op, sop, char) \
        gen_testnotzero(op, sop, __uchar) gen_testnotzero(op, sop, __longlong)                    \
...
#define gen_testreallyall(op, sop) gen_testall(op, sop) gen_test(op, sop, float)

#define gen_testreallyallnotzero(op, sop) \
    gen_testallnotzero(op, sop) gen_testnotzero(op, sop, float)

#define gen_testallunary(op, sop)                                                           \
    gen_testunary(op, sop, int) gen_testunary(op, sop, __uint) gen_testunary(op, sop, char) \
        gen_testunary(op, sop, __uchar) gen_testunary(op, sop, __longlong)                  \
            gen_testunary(op, sop, __ulonglong)

#define gen_testallconst(op, sop, cst)                                         \
    gen_testconst(op, sop, cst, int) gen_testconst(op, sop, cst, __uint)       \
        gen_testconst(op, sop, cst, char) gen_testconst(op, sop, cst, __uchar) \
            gen_testconst(op, sop, cst, __longlong) gen_testconst(op, sop, cst, __ulonglong)

#define gen_testreallyallconst(op, sop, cst) \
    gen_testallconst(op, sop, cst) gen_testconst(op, sop, cst, float)
...
#define run_testreallyall(op, a, b) run_test(op, a, b) test##op##float(a, b);
...
#define run_testreallyallconst(op, a) run_testconst(op, a) testconst##op##float(a);

        gen_testreallyall(add, +) gen_testreallyall(sub, -) gen_testreallyall(
            mul, *) gen_testreallyallnotzero(div, /) gen_testreallyall(lt, <)
            gen_testreallyall(gt, >) gen_testreallyall(le, <=) gen_testreallyall(
                eq, ==) gen_testreallyall(ne, !=) gen_testreallyall(ge, >=)

                gen_testall(land, &&) gen_testall(lor, ||) gen_testall(xor, ^) gen_testall(and, &)
                    gen_testall(or, |) gen_testall(shl, <<) gen_testall(shr,
                                                                        >>) gen_testallnotzero(mod,
                                                                                               %)

                        gen_testreallyallconst(add, +, 42) gen_testreallyallconst(sub, -, 42)
                            gen_testreallyallconst(mul, *, 42) gen_testreallyallconst(div, /, 42)
                                gen_testreallyallconst(lt, <, 42) gen_testreallyallconst(
                                    gt, >, 42) gen_testreallyallconst(le, <=, 42)
                                    gen_testreallyallconst(eq, ==, 42) gen_testreallyallconst(
                                        ne, !=, 42) gen_testreallyallconst(ge, >=, 42)

                                        gen_testallconst(land, &&, 1) gen_testallconst(lor, ||, 1)
                                            gen_testallconst(xor, ^, 42) gen_testallconst(and, &, 42)
                                                gen_testallconst(or, |, 42) gen_testallconst(shl,
                                                                                             <<,
                                                                                             5)
...
#define gen_testcast(from, to, outfn)  \
    to testcast##from##to(from v)      \
    {                                  \
        write("cast from " #from " "); \
        outfn(v);                      \
        write(" to " #to " ");         \
        outfn((to) v);                 \
        OUTC('\n');                    \
...
#define gen_testallcasts(from)                                                                     \
    gen_testcast(from, char, hex) gen_testcast(from, __uchar, hexu) gen_testcast(from, __int, hex) \
        gen_testcast(from, __uint, hexu) gen_testcast(from, float, hex)                            \
            gen_testcast(from, __longlong, dumplong) gen_testcast(from, __ulonglong, dumplong)

gen_testallcasts(char) gen_testallcasts(__uchar) gen_testallcasts(__int) gen_testallcasts(__uint)
    gen_testallcasts(__longlong) gen_testallcasts(__ulonglong) gen_testallcasts(float)
  ...
  exit(0);
    ...
    for (j = 0; j < (sizeof(nums1) / sizeof(int)); j++) {
        for (k = 0; k < (sizeof(nums2) / sizeof(int)); k++) {
            ...
            run_test(xor, nums1[j], nums2[k]) run_test(land, nums1[j], nums2[k]) run_test(lor,
                                                                                          nums1[j],
                                                                                          nums2[k])
                run_test(and, nums1[j], nums2[k]) run_test(or, nums1[j], nums2[k])
                    run_test(shl, nums1[j], shifts2[k]) run_test(shr, nums1[j], shifts2[k])
                        run_testreallyall(add, nums1[j], nums2[k]) run_testreallyall(sub,
                                                                                     nums1[j],
                                                                                     nums2[k])
                            run_testreallyall(mul, nums1[j], nums2[k]) run_testreallyall(lt,
                                                                                         nums1[j],
                                                                                         nums2[k])
                                run_testreallyall(gt,
                                                  nums1[j],
                                                  nums2[k]) run_testreallyall(le, nums1[j], nums2[k])
                                    run_testreallyall(eq,
                                                      nums1[j],
                                                      nums2[k]) run_testreallyall(ne,
                                                                                  nums1[j],
                                                                                  nums2[k])
                                        run_testreallyall(ge, nums1[j], nums2[k]) run_testreallyall(
                                            div,
                                            nums1[j],
                                            nums2[k]) /* has one problem when float dividing 32767/-32768
                                                           resulting in -1 instead of zero */
                run_test(mod, nums1[j], nums2[k])
            //testltfloat(nums1[j], nums2[k]);
        }
        run_testconst(xor, nums1[j]) run_testconst(land, nums1[j]) run_testconst(lor, nums1[j])
            run_testconst(and, nums1[j]) run_testconst(or, nums1[j]) run_testconst(shl, nums1[j])
                run_testconst(shr, nums1[j]) run_testreallyallconst(add, nums1[j])
                    run_testreallyallconst(sub, nums1[j]) run_testreallyallconst(mul, nums1[j])
                        run_testreallyallconst(lt, nums1[j]) run_testreallyallconst(gt, nums1[j])
    ...
    for (i = 0; i < (sizeof(tfu) / sizeof(testufuncptr)); i++) {
        for (j = 0; j < (sizeof(nums1) / sizeof(int)); j++) {
            ...
            write("xchar ");
            tfu[i]((char) nums1[j]);
            run_testunary(not, nums1[j]) run_testunary(lnot, nums1[j])
...
__longlong longschlong(void)
{
    __longlong a = 5;
    if (a == 42)
        return 23; /* triggers VT_JMPI problems */
    a += 3;        //zahl();
    return a;
}
...
void ellprint(char *format, ...)
{
    ...
    while (*format) {
        switch (*format) {
        case 'c':
            hex(va_arg(vl, vachar));
            break;
        case 'i':
            hex(va_arg(vl, vaint));
            break;
        case 'l':
            hexl(va_arg(vl, __longlong));
            break;
        case 'f':
            write("float ");
            hex(va_arg(vl, vafloat));
            break;
        default:
            break;
        }
